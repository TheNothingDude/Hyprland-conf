class Konyv:
    ...
    def hossz(self):
        if self.oldalszam <200:
            return "rövid"
        elif self.oldalszam >=200 and self.oldalszam <=600:
            return "közép"
        else:
            return "hosszú"
    
konyvek = []    
with open("./konyvek-adatok.txt" ,encoding="UTF-8") as s:
    s.readline()
    for i in s:
...
def f5(list):
    with open("./negyezer-export.txt", "w", encoding="UTF-8") as out:
        for i in list:
            if i.eldasar == 4000:
                out.write(f"{i.cim} ({i.hossz()}) \n")
